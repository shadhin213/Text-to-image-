  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="bg-gray-800 p-6 rounded-xl shadow-lg">
        ...
        <div className="mb-6">
            <label className="block text-sm font-medium text-gray-300 mb-2">Style Category</label>
            <div className="flex flex-wrap gap-2">
                {categories.map(cat => (
                    <button key={cat} onClick={() => setCategory(cat)} disabled={isLoading} className={`px-4 py-2 text-sm font-medium rounded-full transition-colors duration-200 ${category === cat ? 'bg-indigo-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}>
                        {cat}
                    </button>
                ))}
            </div>
        ...
        <div className="mb-6">
            ...
            <div className="flex flex-wrap gap-2">
                {aspectRatios.map(ar => (
                    <button key={ar.value} onClick={() => handleAspectRatioChange(ar.value)} disabled={isLoading} className={`px-4 py-2 text-sm font-medium rounded-full transition-colors duration-200 ${aspectRatio === ar.value ? 'bg-indigo-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}>
                        {ar.label} <span className="text-gray-400 text-xs">({ar.value})</span>
                    </button>
                ))}
            </div>
        </div>

        <p className="text-gray-400 mb-4">Describe the image you want to create. The more detailed your prompt, the better the result.</p>
        <div className="flex flex-col sm:flex-row gap-4">
          <input
            ...
            className="flex-grow bg-gray-700 border border-gray-600 text-white rounded-md px-4 py-3 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition"
            disabled={isLoading}
          />
          <button
        ...
        <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {isLoading && (
            imageStyles.map(style => (
              <div key={style} className="aspect-square flex justify-center items-center bg-gray-800/50 rounded-lg border-2 border-dashed border-gray-700">
                <div className="text-center">
                  <Spinner size="lg"/>
                  <p className="mt-4 text-gray-400 text-sm">{style}...</p>
                </div>
              </div>
            ))
          )}
          ...
          {!isLoading && imageUrls.length === 0 && (
             <div className="sm:col-span-2 lg:col-span-4 text-center text-gray-500 bg-gray-800/50 rounded-lg py-20 border-2 border-dashed border-gray-700">
                ...
                <p className="mt-2">Your 4 generated images will appear here.</p>
              </div>
          )}
        </div>
      </div>
